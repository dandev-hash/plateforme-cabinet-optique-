  ...
  <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden 
    ...
    <div class="relative overflow-hidden h-48 sm:h-56 bg-gray-50 flex items-center justify-center">
      ...
      <div class="absolute top-3 right-3 flex space-x-2 z-10">
        <!-- Badge PROMO (Rouge) -->
        <span v-if="product.isPromo" class="px-3 py-1 text-xs font-bold bg-red-500 text-white rounded-full shadow-md">
          -{{ product.promoPercentage || 'XX' }}%
        </span>
        <!-- Badge NOUVEAU (Vert clair du thème) -->
        <span v-if="product.isNew" class="px-3 py-1 text-xs font-bold bg-green-500 text-white rounded-full shadow-md">
          NEW
        </span>
      </div>
