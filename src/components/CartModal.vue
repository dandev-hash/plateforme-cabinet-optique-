<template>
  ...
  <transition name="fade">
    <div v-if="isVisible" class="fixed inset-0 z-[90] transition-opacity bg-black/30" @click="emit('close')">
      ...
      <transition name="slide-in">
        <div v-if="isVisible"
          ...
          :class="{ 'translate-x-0': isVisible, 'translate-x-full': !isVisible }" @click.stop>
          <!-- Empêche la fermeture si l'on clique à l'intérieur -->

          <!-- En-tête du Panier -->
          <div
            class="sticky top-0 bg-white/90 backdrop-blur-sm p-4 flex justify-center items-center z-10 flex-shrink-0 relative">
                ...
                <div class="ml-4 flex flex-1 flex-col">
                  ...
                  <div class="flex flex-1 items-end justify-between text-sm mt-3 pt-2 border-t border-gray-100">
                    ...
                    <div class="flex items-center space-x-1">
                      <button @click="handleUpdateQuantity(item.id, item.quantity - 1)" :disabled="item.quantity <= 1"
                    ...
                    <!-- Bouton Retirer -->
                    <div class="flex">
                      <button @click="emit('removeItem', item.id)" type="button"
          ...
          <!-- Pied de Page -->
          <div class="flex-shrink-0 bg-white/90 backdrop-blur-sm shadow-2xl p-3 sm:p-4">
            ...
            <div v-if="totalSavings > 0" class="flex justify-between text-sm font-medium text-green-600 mb-3">
              <p>Vous économisez</p>
            ...
            <div class="mt-3 space-y-2">
              <button @click="goToCheckout" :disabled="cartItems.length === 0"
                class="flex items-center justify-center rounded-xl border border-transparent bg-green-600 px-4 py-2.5 text-base font-medium text-white shadow-md hover:bg-green-700 transition duration-300 w-full disabled:opacity-50 disabled:cursor-not-allowed">
                Passer à la caisse
              </button>
              <button @click="cartStore.clearCart" :disabled="cartItems.length === 0"
                class="flex items-center justify-center rounded-xl border border-red-300 bg-white px-4 py-2.5 text-base font-medium text-red-600 shadow-md hover:bg-red-50 transition duration-300 w-full disabled:opacity-50 disabled:cursor-not-allowed">
                Vider le panier
              </button>
            </div>
          </div>
...
const handleRemoveItem = (id) => cartStore.removeItem(id);
const handleUpdateQuantity = (id, q) => cartStore.updateQuantity(id, q);
